package com.zigaai.authentication.security.oauth2;
...
public final class RedisOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public void save(OAuth2Authorization authorization) {
        ...
        if (authorizationCode != null) {
            ...
            redisTemplate.opsForValue().set(OAuth2RedisKeys.AUTHORIZATION_CODE.apply(authorizationCode.getToken().getTokenValue()), authorization.getId(), authorizationCodeTimeToLive, TimeUnit.SECONDS);
        }
        ...
        if (accessToken != null) {
            ...
            redisTemplate.opsForValue().set(OAuth2RedisKeys.ACCESS_TOKEN.apply(accessToken.getToken().getTokenValue()), authorization.getId(), accessTokenTimeToLive, TimeUnit.SECONDS);
        }
        ...
        if (refreshToken != null) {
            ...
            redisTemplate.opsForValue().set(OAuth2RedisKeys.REFRESH_TOKEN.apply(refreshToken.getToken().getTokenValue()), authorization.getId(), refreshTokenTimeToLive, TimeUnit.SECONDS);
        }
        ...
        if (oidcIdToken != null) {
            ...
            redisTemplate.opsForValue().set(OAuth2RedisKeys.OIDC_TOKEN.apply(oidcIdToken.getToken().getTokenValue()), authorization.getId(), accessTokenTimeToLive, TimeUnit.SECONDS);
        }

        String authorizationState = authorization.getAttribute(OAuth2ParameterNames.STATE);
        if (StringUtils.hasText(authorizationState)) {
            redisTemplate.opsForValue().set(OAuth2RedisKeys.OAUTH2_STATE_CODE.apply(authorizationState), authorization.getId(), refreshTokenTimeToLive, TimeUnit.SECONDS);
        }
        redisTemplate.opsForValue().set(OAuth2RedisKeys.RESOURCE_PRINCIPAL.apply(new PrincipalKey(authorization.getPrincipalName(), authorization.getId())), authorization.getRegisteredClientId(), refreshTokenTimeToLive, TimeUnit.SECONDS);
        redisTemplate.opsForValue().set(OAuth2RedisKeys.OAUTH2_AUTHORIZATION.apply(authorization.getId()), authorization, refreshTokenTimeToLive, TimeUnit.SECONDS);
    }
