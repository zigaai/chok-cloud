...
public final class JWTUtil {
    ...
    public static PayloadDTO parseVerified(String token, String salt) throws ParseException, JsonProcessingException, JOSEException {
        ...
        JWSObject jwsObject = pair.getFirst();
        PayloadDTO payload = pair.getSecond();
        check(jwsObject, pair.getSecond(), salt);
        return payload;
    }
    ...
    private enum INNER {
        ...
        private final KeyStoreKeyFactory KEY_STORE_KEY_FACTORY;

        private final KeyPair KEY_PAIR;

        INNER() {
            KEY_STORE_KEY_FACTORY = new KeyStoreKeyFactory(new ClassPathResource(SystemConstant.JKS_ALIAS + ".jks"), SystemConstant.JKS_PASSWORD.toCharArray());
